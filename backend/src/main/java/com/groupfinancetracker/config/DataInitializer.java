...
public class DataInitializer {
    ...
    CommandLineRunner init(UserService userService,
        ...
        return args -> {
            UserResponse alice = userService.create(new CreateUserRequest("Alice", "alice@example.com", "password123"));
            UserResponse bob = userService.create(new CreateUserRequest("Bob", "bob@example.com", "password123"));
            UserResponse carol = userService.create(new CreateUserRequest("Carol", "carol@example.com", "password123"));

            GroupResponse grp = groupService.create(new CreateGroupRequest("Goa Trip Group", alice.id(), Set.of(bob.id(), carol.id())));

            EventResponse evt = eventService.create(new CreateEventRequest(grp.id(), "Goa Trip", alice.id(), LocalDate.now()));
