...
public class EventService {
    ...
    public DtoModels.EventResponse create(DtoModels.CreateEventRequest req) {
        ...
        // Assign custom week/year aligned with SubEventService so events appear in weekly view immediately
        if (e.getEventDate() != null) {
            LocalDate base = subEventRepository.findEarliestSubEventDate();
            if (base == null) base = e.getEventDate();
    ...
    public List<DtoModels.EventResponse> listByGroupWeek(@NonNull Long groupId, @NonNull Integer weekNumber, @NonNull Integer year) {
        if (!groupRepository.existsById(groupId)) throw new NotFoundException("Group not found: " + groupId);
        Set<Long> eventIds = new HashSet<>();
        var fromSubs = subEventRepository.findEventIdsByGroupAndWeek(groupId, weekNumber, year);
        if (fromSubs != null) eventIds.addAll(fromSubs);
        var fromEvents = eventRepository.findByGroup_IdAndWeekNumberAndYear(groupId, weekNumber, year);
        if (fromEvents != null) fromEvents.forEach(ev -> eventIds.add(ev.getId()));
    ...
    public List<DtoModels.WeekSummary> listWeeks(@NonNull Long groupId) {
        ...
        for (Object[] arr : subEventRepository.listDistinctWeeksAscByGroup(groupId)) {
            ...
            if (ids != null) weekEvents.get(key).addAll(ids);
        }
        ...
        for (Map.Entry<String, java.util.Set<Long>> e : weekEvents.entrySet()) {
            ...
            out.add(new DtoModels.WeekSummary(Integer.parseInt(parts[0]), Integer.parseInt(parts[1]), (long) e.getValue().size()));
        }
        out.sort(Comparator.comparing(DtoModels.WeekSummary::year).reversed().thenComparing(DtoModels.WeekSummary::weekNumber, Comparator.reverseOrder()));
    ...
    public DtoModels.NewEventWarningResponse newEventWarning(@NonNull Long groupId, java.time.LocalDate date) {
        ...
        for (int[] yw : weeks) {
            ...
            if (weekKeys.add(key)) deduped.add(yw);
        }
        // check presence
        ...
        // Identify the week that would be dropped: the earliest (min year, then min week)
        int[] earliest = null;
        for (int[] yw : deduped) {
        ...
        var shares = shareRepository.findBySubEvent_Event_Group_IdAndSubEvent_WeekNumberAndSubEvent_Year(groupId, dropWeek, dropYear);
        int pending = 0;
        for (var s : shares) {
            if (s.getPaymentStatus() == null || s.getPaymentStatus().getStatus() != com.groupfinancetracker.entity.PaymentState.CONFIRMED) {
                pending++;
            }
        }
        if (pending > 0) {
            String msg = "Adding this event creates a new page and hides the oldest page. There are " + pending + " pending payments there.";
